import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Kontakt",
  description:
    "Ta kontakt med Skudeneshavn Næringsforening.",
};

export default function Kontakt() {
  return (
    <>
      {/* Hero */}
      <section className="bg-navy text-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="text-gold font-semibold tracking-widest uppercase text-sm mb-3">
            Kontakt
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold mb-6">
            Ta kontakt med oss
          </h1>
          <p className="text-white/70 text-lg max-w-2xl">
            Vi hører gjerne fra deg — enten du vil bli medlem, samarbeide,
            eller har spørsmål om vårt arbeid.
          </p>
        </div>
      </section>

      <section className="py-16 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Kontaktinfo */}
            <div>
              <h2 className="text-2xl font-bold text-navy mb-8">
                Kontaktinformasjon
              </h2>

              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-sand rounded-lg flex items-center justify-center shrink-0">
                    <svg className="w-5 h-5 text-navy" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-semibold text-navy">E-post</p>
                    <a
                      href="mailto:Skude.nf@gmail.com"
                      className="text-sea-blue hover:underline"
                    >
                      Skude.nf@gmail.com
                    </a>
                  </div>
                </div>

                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-sand rounded-lg flex items-center justify-center shrink-0">
                    <svg className="w-5 h-5 text-navy" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-semibold text-navy">Adresse</p>
                    <p className="text-navy/60">Kaigaten 105, Skudeneshavn</p>
                  </div>
                </div>
              </div>

              {/* For media */}
              <div className="mt-10 bg-sand-light rounded-xl p-6">
                <h3 className="font-bold text-navy mb-2">For media</h3>
                <p className="text-sm text-navy/60 mb-3">
                  Skudeneshavn Næringsforening stiller gjerne til intervju og
                  kommentar om næringsliv, byutvikling og stedsutvikling på
                  sør-Karmøy. Kontakt styreleder for pressehenvendelser.
                </p>
                <a
                  href="mailto:Skude.nf@gmail.com"
                  className="text-sm text-sea-blue font-medium hover:underline"
                >
                  Skude.nf@gmail.com
                </a>
              </div>

              {/* Sosiale medier */}
              <div className="mt-8">
                <h3 className="font-bold text-navy mb-4">Følg oss</h3>
                <div className="flex gap-4">
                  <a
                    href="https://www.facebook.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 bg-navy/5 rounded-lg flex items-center justify-center hover:bg-navy/10 transition-colors"
                    aria-label="Facebook"
                  >
                    <svg className="w-5 h-5 text-navy" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
                    </svg>
                  </a>
                  <a
                    href="https://www.instagram.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 bg-navy/5 rounded-lg flex items-center justify-center hover:bg-navy/10 transition-colors"
                    aria-label="Instagram"
                  >
                    <svg className="w-5 h-5 text-navy" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" />
                    </svg>
                  </a>
                </div>
              </div>
            </div>

            {/* Kontaktskjema */}
            <div>
              <h2 className="text-2xl font-bold text-navy mb-8">
                Send oss en melding
              </h2>
              <form className="space-y-5">
                <div>
                  <label htmlFor="navn" className="block text-sm font-medium text-navy mb-1">
                    Navn
                  </label>
                  <input
                    type="text"
                    id="navn"
                    name="navn"
                    className="w-full rounded-md border border-navy/20 px-4 py-2.5 text-sm focus:border-gold focus:ring-1 focus:ring-gold outline-none"
                    placeholder="Ditt navn"
                  />
                </div>
                <div>
                  <label htmlFor="epost" className="block text-sm font-medium text-navy mb-1">
                    E-post
                  </label>
                  <input
                    type="email"
                    id="epost"
                    name="epost"
                    className="w-full rounded-md border border-navy/20 px-4 py-2.5 text-sm focus:border-gold focus:ring-1 focus:ring-gold outline-none"
                    placeholder="din@epost.no"
                  />
                </div>
                <div>
                  <label htmlFor="emne" className="block text-sm font-medium text-navy mb-1">
                    Emne
                  </label>
                  <select
                    id="emne"
                    name="emne"
                    className="w-full rounded-md border border-navy/20 px-4 py-2.5 text-sm focus:border-gold focus:ring-1 focus:ring-gold outline-none bg-white"
                  >
                    <option value="">Velg emne</option>
                    <option value="medlemskap">Medlemskap</option>
                    <option value="samarbeid">Samarbeid</option>
                    <option value="arrangement">Arrangement</option>
                    <option value="presse">Pressehenvendelse</option>
                    <option value="annet">Annet</option>
                  </select>
                </div>
                <div>
                  <label htmlFor="melding" className="block text-sm font-medium text-navy mb-1">
                    Melding
                  </label>
                  <textarea
                    id="melding"
                    name="melding"
                    rows={5}
                    className="w-full rounded-md border border-navy/20 px-4 py-2.5 text-sm focus:border-gold focus:ring-1 focus:ring-gold outline-none resize-none"
                    placeholder="Skriv din melding her..."
                  />
                </div>
                <button
                  type="submit"
                  className="w-full bg-gold text-navy py-3 rounded-md font-semibold hover:bg-gold-light transition-colors"
                >
                  Send melding
                </button>
              </form>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
